...
import {Export, MaterializeIt, FileDescriptor} from "@affinity-lab/util";

@Entity({name: attachmentConfig.table})
export default class Attachment extends Atom {
	@Export() id: number;
	@Column() private fileName: string
	@Export() @Column() mimeType: string
	@Export() @Column() size: number
	@Export() @Column({default: null}) title: string | null;
	@Export() @Column({type: "int", unsigned: true}) position: number;
	@Export() @Column({type: 'json', default: null}) image: Image | null
	@Column({update: false}) entity: string;
	@Column({update: false,}) collection: string;
	@Column({update: false, unsigned: true}) ownerId: number;
	...
	get path(): string {return storage.pathStrategy.createPath(this);}
	get fullPath(): string {return storage.fullPath(this);}

	@MaterializeIt() get ownerItem(): Promise<Atom> { return this.collectionDescriptor.atom.pick(this.ownerId)}

	get collectionDescriptor(): CollectionDescriptor {return storage.collections[this.owner.cid];}
	...
	async rename(name: string) {
		...
		await storage.renameFile(this, name);
		this.fileName = name;
		await this.save();
	}

	async delete(reorder: boolean = true) {
		await storage.deleteFile(this);
		await this.remove();
	...
	static async factory(file: FileDescriptor, owner: CollectionOwner): Promise<Attachment> {
		...
		await storage.storeFile(file.file, attachment);
		return attachment;
	}
	...
	static async findOfOwner(owner: CollectionOwner, fileName: string): Promise<Attachment[]> {
		return this.createQueryBuilder().where("fileName LIKE :fileName", {fileName}).andWhere(owner).orderBy({position: "ASC"}).getMany();
	}
	static async pickOfOwner(owner: CollectionOwner, id: number): Promise<Attachment | null> { return this.findOne({where: {...owner, id}});}

	@AfterUpdate() afterUpdate() { storage.eventSource.emit(new AttachmentUpdated(this.id));}
	@AfterRemove() afterRemove() { storage.eventSource.emit(new AttachmentRemoved(this.id));}
